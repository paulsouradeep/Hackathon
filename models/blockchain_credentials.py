...
Provides tamper-proof credential storage and instant verification
"""
...
class BlockchainCredentialVerifier:
    """
    Blockchain-inspired credential verification system
    Features:
    ...
    def __init__(self, db_path: str = "credentials_blockchain.db"):
        ...
        logger.info("Blockchain Credential Verifier initialized")
    ...
    def init_blockchain_db(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS verification_requests (
                ...
                institution_id TEXT,
                status TEXT DEFAULT 'pending', -- pending, verified, rejected, fraud_detected
    ...
    def _create_genesis_block(self):
        ...
        if cursor.fetchone()[0] == 0:
            ...
            signature = self._sign_data(genesis_hash)
            
            cursor.execute('''
                INSERT INTO credential_blocks 
                (previous_hash, timestamp, credential_data, block_hash, digital_signature, merkle_root, nonce)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', ("0", time.time(), json.dumps(genesis_data), genesis_hash, signature, genesis_hash, 0))
    ...
    def add_credential_to_blockchain(self, credential_data: Dict) -> str:
        ...
        cursor.execute('''
            ...
            (previous_hash, timestamp, credential_data, block_hash, digital_signature, merkle_root, nonce)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (previous_hash, time.time(), data_string, block_hash, signature, merkle_root, nonce))
    ...
    def verify_credential(self, credential_hash: str) -> Dict:
        ...
        cursor.execute('''
            SELECT credential_data, digital_signature, previous_hash, nonce, timestamp
        ...
        credential_data, signature, previous_hash, nonce, timestamp = block_data
        ...
        # Verify digital signature
        if not self._verify_signature(credential_hash, signature, self.public_key):
            return {
    ...
    def _calculate_verification_score(self, credential_data: Dict) -> float:
        ...
        # Recency bonus
        timestamp = credential_data.get('timestamp', 0)
        age_days = (time.time() - timestamp) / (24 * 3600)
        if age_days < 365:  # Less than 1 year old
            score += 10.0
        elif age_days < 1825:  # Less than 5 years old
            score += 5.0
    ...
    def add_verified_institution(self, institution_data: Dict) -> bool:
        ...
        try:
            cursor.execute('''
                ...
                (institution_id, name, type, public_key, country, accreditation_body)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                institution_data['institution_id'],
                institution_data['name'],
                institution_data['type'],
                institution_data.get('public_key', ''),
    ...
    def detect_credential_fraud(self, candidate_id: str, submitted_credentials: List[Dict]) -> Dict:
        ...
        for credential in submitted_credentials:
            ...
            if institution_id and not self._validate_institution(institution_id):
                fraud_indicators.append({
            ...
            if not self._check_temporal_consistency(credential):
                fraud_indicators.append({
                    "type": "temporal_inconsistency",
                    "severity": "medium",
                    "description": "Credential dates are inconsistent"
                })
    ...
    def _validate_institution(self, institution_id: str) -> bool:
        """Validate if institution exists and is verified"""
        if not institution_id:
            return False
        return self._is_institution_verified(institution_id)
    
    def _check_temporal_consistency(self, credential: Dict) -> bool:
        """Check if credential dates are consistent"""
        start_date = credential.get('start_date')
        end_date = credential.get('end_date')
        ...
        try:
            ...
            # If issue date exists, it should be after end date
            if issue_date:
    ...
    def _validate_credential_format(self, credential: Dict) -> bool:
        ...
        required_fields = ['type', 'title', 'institution_id']
    ...
    def bulk_verify_credentials(self, credential_hashes: List[str]) -> Dict:
        ...
        return {
            ...
            "bulk_verification_timestamp": datetime.now().isoformat()
        }

# Initialize blockchain credential verifier
blockchain_verifier = BlockchainCredentialVerifier()
